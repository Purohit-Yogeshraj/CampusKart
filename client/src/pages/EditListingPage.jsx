        ...
        <div className="sell-form">
          ...
          <form id="edit-ad-form" onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <input type="number" name="price" className="form-control" value={form.price} onChange={handleChange} min="1" required />
            </div>
            <div className="form-group">
              <label><i className="fas fa-phone"></i> Phone Number</label>
              <input type="tel" name="phone" className="form-control" value={form.phone} onChange={handleChange} pattern="[0-9]{10}" required />
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              <textarea name="description" className="form-control" rows="4" value={form.description} onChange={handleChange} required></textarea>
            </div>
            <div className="form-group">
              <label><i className="fas fa-image"></i> Current Image</label>
              {form.imagePath || preview ? (
                <img
                  src={preview || (form.imagePath.startsWith("/images/") ? form.imagePath : `${SERVER_BASE_URL}${form.imagePath}`)}
                  alt="Current"
                  style={{ maxWidth: "100%", maxHeight: "200px", borderRadius: "8px", margin: "10px 0" }}
                />
              ) : (
                <div style={{ color: "#7f8c8d", margin: "10px 0" }}>No image uploaded</div>
              )}
              <input type="file" className="form-control" accept="image/jpeg,image/png" onChange={handleImageChange} />
            </div>
            <button type="submit" className="btn-submit">Update Ad</button>
            <Link to="/sell" style={{ display: "block", textAlign: "center", marginTop: "15px", color: "#2b84ea" }}>
              ← Back to My Ads
            </Link>
          </form>
        </div>
